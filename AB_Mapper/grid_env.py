...
class GridEnv:
    ...
    def reset(self):
        ...
        self.background_grid = np.zeros(shape=(self.row, self.col), dtype=int)  # [0,0,...,0]
        self.background_grid = self.update_background_grid(self.background_grid)  # OBS=1 [0,1,...,0]
        self.background_img = np.ones(shape=(self.row * self.tilesize, self.col * self.tilesize, 3),
                                      dtype=np.uint8) * 255
        # Initialize a map to store all the agents' positions. -1 represents current grid is not occupied, otherwise represents the agent index.
        self.agent_map = -np.ones(self.background_grid.shape)
        ...
        # initialize dynamic obstacles
        self.robot_manager = RobotManager(self.background_grid.copy(), self.idx_to_object, self.dynamic_obs_num,
                                          self.detect_agent)
        self.robot_manager.init_robots(current_grid)
        self.robot_pose = self.robot_manager.robot_pose
        self.robot_future_traj = self.robot_manager.robot_future_traj
    ...
    def step(self, action):
        ...
        if self.dynamic_obs_num != 0:
            self.resolve_robot_conflict(current_grid, robot_map, current_robot_map, agent_map, current_agent_map,
                                        current_robot_pose, current_agent_pose)

        self.resolve_agent_conflict(current_grid, robot_map, current_robot_map, agent_map, current_agent_map,
                                    current_robot_pose, current_agent_pose)
    ...
    def init_agents(self):
        ...
        for i in range(self.agent_num):
            goal = self.sample_free_space(current_grid)
            while (len(self.agent_goal) > 0 and any(np.array_equal(goal, x) for x in self.agent_goal)):
                goal = self.sample_free_space(current_grid)  # FANGZ防止目标点重复，再次设置
            self.agent_goal.append(goal)
            # print("goal: ", goal)
            y, x = goal
            xmin = max(int(x - self.goal_sample_range), 0)
            ...
            # start_pose = self.sample_free_space(current_grid[xmin:xmax, ymin:ymax])\
            #              +np.array((ymin, xmin))
            start_pose = self.sample_free_space(current_grid[7:13, 7:13])+np.array((8,8))

            # if np.random.uniform(0, 1) <= 0.5:
            #     start_pose = self.sample_free_space(current_grid[int(xmin / 3):int(xmax), int(ymin / 3):int(ymax)])
            #     # + np.array((ymin, xmin))
            #     # print('start_pose', start_pose)
            # else:
            #     start_pose = self.sample_free_space(current_grid[int(xmax / 2):xmax, int(ymax / 2):ymax])
                # + np.array((ymin, xmin))
            #     print('new start_pose', start_pose)
            self.agent_pose.append(start_pose)
            self.set_grid(current_grid, start_pose, "agent")
            self.agent_map[start_pose[1], start_pose[0]] = i
            agent = Agent(start_pose, goal, i)
    ...
    def sample_free_space(self, grid):
        ...
        @param [array] grid : grid map with obstacle information.
        '''
        idx_free = np.argwhere(grid == self.object_to_idx["free"])  # 返回非0的数组元组的索引，其中a是要索引数组的条件
        sample = np.random.randint(idx_free.shape[0])
        return idx_free[sample][::-1]
    ...
    def resolve_robot_conflict(self, current_grid, robot_map, current_robot_map, agent_map, current_agent_map,
                               current_robot_pose, current_agent_pose):
        # resolve robots conflict first
        for i in range(self.dynamic_obs_num):
            robot_pose_i_last = self.robot_pose[i]
            ...
            if not collision_flag:
                ...
                if idx_robot.size > 0:
                    for j in idx_robot:
                        ...
                        robot_pose_j_now = current_robot_pose[int(j)]
                        # If swap position occured, stay at original position
                        if (robot_pose_i_now == robot_pose_j_last).all() and (
                                robot_pose_i_last == robot_pose_j_now).all():
                            collision_flag = True
                            break
                        # if go to the same position
                        elif (robot_pose_i_now == robot_pose_j_now).all():
    ...
    def resolve_agent_conflict(self, current_grid, robot_map, current_robot_map, agent_map, current_agent_map,
                               current_robot_pose, current_agent_pose):
        # resolve agents conflict
        for i in range(self.agent_num):
            if self.agent_done[i]:
            ...
            if not collision_flag:
                ...
                if idx_robot.size > 0:
                    for j in idx_robot:
                        ...
                        robot_pose_j_now = current_robot_pose[int(j)]
                        # If swap position occured, stay at original position
                        if (agent_pose_i_now == robot_pose_j_last).all() and (
                                agent_pose_i_last == robot_pose_j_now).all():
                            collision_flag = True
                            break
                        # if go to the same position
                        elif (agent_pose_i_now == robot_pose_j_now).all():
            ...
            if not collision_flag:
                ...
                if idx_agent.size > 0:
                    for j in idx_agent:
                        ...
                        agent_pose_j_now = current_agent_pose[int(j)]
                        # If swap position occured, stay at original position
                        if (agent_pose_i_now == agent_pose_j_last).all() and (
                                agent_pose_i_last == agent_pose_j_now).all():
                            collision_flag = True
                            collision_agent_flag = True
                            break
                        # if go to the same position
...
#     # def render(self, sub_agent_list,show_traj = False, dynamic_obs_traj = False, agent_i=3,traj = [], save_img = False, save_img_dir = "storage"):
#     #     '''
#     #     @param [boolean] show_traj : Determine if we render the agents' trajactories.
#     #     @param [boolean] dynamic_obs : TODO, remove this param
#     #     Return the rendered image.
...
#     #     self.renderer.draw_agents(img, self.agent_pose, self.agent_on_goal)
#     #     #self.renderer.draw_obs(img, self.agent_pose, self.visibility)
#     #     self.renderer.draw_attention(img,self.agent_pose,agent_i,sub_agent_list)
#     #
#     #     if save_img:
#     #         new_img = Image.fromarray(img)
#     #         img_save_path = os.path.join(save_img_dir, str(self.time)+".png")
...
#     def render(self, show_traj=False, dynamic_obs_traj=False,  traj=[], save_img=False,
#                save_img_dir="storage"):
#         '''
#         @param [boolean] show_traj : Determine if we render the agents' trajactories.
#         @param [boolean] dynamic_obs : TODO, remove this param
#         Return the rendered image.
